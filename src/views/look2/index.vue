<template>
  <div id="look-sec" class="wrap-look">
    ...
    <div class="info scroll">
      ...
      <form action="index_submit" @submit.prevent="gotoNext" method="get" accept-charset="utf-8">
        <div class="form-line">
          <label for="">标题</label>
          <input type="text" v-model="formData.title" disabled>
        </div>
        <div class="form-line">
          <label for="">作品简介</label>
          <textarea v-model="formData.textarea" name="" id="" cols="30" rows="5" disabled></textarea>
        </div>
        <div class="form-line fileUp">
          <label for="">封面</label>
          <input disabled class="image" type="file" name="file" v-on:change="selectImg" accept="image/png,image/gif,image/jpg">
          <div class="fileAdd">添加</div>
          <div class="cover">
            <img src="../../../ossweb-img/image/bg.jpg" height="829" width="1920" alt="">
            <div class="cover"></div>
          </div>
        </div>
        <div class="form-line fileUp">
          <label for="">PPT</label>
          <input disabled type="file" name="file" accept="application/vnd.ms-powerpoint" v-on:change="selectPPT">
          <div class="fileAdd">添加</div>
          <p class="instructions">支持PDF格式 ，文件名称只支持英文和数字命名</p>
        </div>
        <div class="form-line fileUp">
          <label for="">视频</label>
          <input disabled type="text" v-model="formData.video">
          <p class="instructions">复制腾讯视频分享</p>
        </div>
        <div class="form-line fileUp">
          <label for="">DEMO</label>
          <input disabled type="file" name="file" accept="application/x-zip-compressed" v-on:change="selectFile">
          <div class="fileAdd">添加</div>
